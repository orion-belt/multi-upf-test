...
SGI_IPV4_SEC=$(ifconfig $INTERFACE_CORE_SEC | grep "inet " | awk '{print $2}')
...
N6_IPV4_ADDRESS_LOCAL_SEC=${N6_IPV4_ADDRESS_LOCAL_SEC:-$(ifconfig $INTERFACE_CORE_SEC | grep "inet " | awk '{print $2}' | cut -d"." -f1-3)".202"}
...
N6_IPV4_ADDRESS_REMOTE_SEC=${N6_IPV4_ADDRESS_REMOTE_SEC:-$(ifconfig $INTERFACE_CORE_SEC | grep "inet " | awk '{print $2}' | cut -d"." -f1-3)".1"}
###############################
# UPF Config
###############################
cp /openair-upf/etc/init_lbo.conf /openair-upf/etc/init.conf
#array=(${CONFIG_DIR}/*.conf ${CONFIG_DIR}/*.json ${BIN_DIR}/nrf_client.sh)
array=(${CONFIG_DIR}/startup_debug.conf ${CONFIG_DIR}/init.conf)
for c in "${array[@]}"; do
...
ip link set $INTERFACE_CORE_SEC down
ip link set $INTERFACE_CORE_SEC name n6-lbo
ip link set n6-lbo up

ip route add $NETWORK_UE_IP via $SGI_IPV4 dev n6
ip route add $NETWORK_UE_IP_LBO via $SGI_IPV4_SEC dev n6-lbo
